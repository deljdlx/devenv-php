...
# Enable Xdebug for PHP-FPM by renaming 99-xdebug.ini.disabled -> 99-xdebug.ini
...
for v in "${versions[@]}"; do
	...
	src="${dir}/99-xdebug.ini.disabled"
	dst="${dir}/99-xdebug.ini"

	if [ -f "$src" ]; then
		echo "Enabling Xdebug for php-fpm${v}: $src -> $dst"
		mv "$src" "$dst"
		changed+=("$v")
	else
		if [ -f "$dst" ]; then
			echo "Xdebug already enabled for php-fpm${v} (found $dst)"
		else
...
echo "Xdebug enabled for: ${changed[*]}"
